...
for line in sys.stdin:
    ...
    # get log prob for class priors if it exists in the dictionary
    logpHam, logpSpam = MODEL.get('ClassPriors', (0,0))
    #loop through words
    for word in words:
        # get log prob for each word if it exists in dictionary
        #ignore others
        (wrd_logpHam, wrd_logpSpam)=MODEL.get(word, (0,0))
        #increment variables
        logpHam += wrd_logpHam
        logpSpam += wrd_logpSpam
    #if log prob of class ham is greater than spam, then have pred class as 0 else 1
    if logpHam >= logpSpam:
        pred_class = 0
    else:
        pred_class = 1
